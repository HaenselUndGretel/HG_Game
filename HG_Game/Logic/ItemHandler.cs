			...
			foreach (Collectable col in pScene.Collectables)
			{
				if (col.IsVisible)
				{
					if (col.CollisionBox.Intersects(pHansel.CollisionBox))
					{
						...
						col.CollisionBox.Intersects(new Rectangle((int)(pGretel.SkeletonPosition.X + Hardcoded.LegUp_OffsetGretel_0.X - 10), (int)(pGretel.SkeletonPosition.Y + Hardcoded.LegUp_OffsetGretel_0.Y - 10), 20, 20))
						)
						)
					{
						delCol = col;
